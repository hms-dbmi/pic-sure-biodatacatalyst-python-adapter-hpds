...
.listPaths()        %s
    ...
    def displayResults(self):
        def __transform__(df):
            df["Concept Path"] = df["Concept Path"].apply(
                lambda x: x.replace("\\", "\\\\")
            )
            return df

        results = map(self.__extractResult__, self.results["results"]["searchResults"])
        resultsDF = pd.DataFrame.from_records(
            list(results),
            columns=[
                "Concept Path",
                "Group Description",
                ...
                "Data Type",
                '<span style="text-align:left">Values</span>',
            ],
        )
    ...
    def listPaths(self):
        results = map(
            self.__path__,
            map(self.__extractResult__, self.results["results"]["searchResults"]),
        )
    ...
    def __extractResult__(self, entry):
        ...
        return [
            conceptPath,
            __leftAlign__(metadata["derived_group_description"]),
            __leftAlign__(metadata["derived_var_description"]),
            dataType,
            __ul__(values),
        ]
