...
class PicSureDictionary:
    ...
    def __init__(self, refHpdsResourceConnection):
        ...
        if "queryScopes" in refHpdsResourceConnection._profile_info:
            self._profile_queryScopes = refHpdsResourceConnection._profile_info[
                "queryScopes"
            ]
        else:
        ...
        self._included_studies = list(
            map(stripSlashes, filter(r.match, self._profile_queryScopes))
        )
    ...
    def genotype_annotations(self):
        ...
        for annotation, annotation_value in results["results"]["info"].items():
            ...
            record["description"] = re.sub(
                '^"|"$', "", record["description"].replace("Description=", "")
            )
            record["values"] = ", ".join(record["values"])
            annotations.append(record)
        df = pd.DataFrame.from_records(annotations)
        df = df.reindex(
            columns=["genomic_annotation", "description", "values", "continuous"]
        )
        return df
    ...
    def find(self, term=None):
        ...
        results["results"]["searchResults"] = list(
            filter(isInScope, results["results"]["searchResults"])
        )
